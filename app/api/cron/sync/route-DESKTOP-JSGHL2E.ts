...
import { GM_RAW, RawMatch } from "@/lib/matches";
...
export async function GET(req: NextRequest) {
  ...
  // Fetch finished fixtures for WC 2026 (league ID 1 = World Cup)
  ...
  for (const fix of fixtures) {
    ...
    const gm = GM_RAW.find((m: RawMatch) => m.home === home && m.away === away);
    if (!gm) continue;
